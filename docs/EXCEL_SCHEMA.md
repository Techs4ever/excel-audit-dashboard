...
| حالة الالتزام بالمتطلبات | `حالة الالتزام وفقًا لإدارة الالتزام` / `حالة الالتزام` (Excel aliases) |
| فئة الضوابط الرقابية | alias `تصنيف الخطر` only when the control-family column is absent |

### Optional columns (10)

| Arabic header | Notes |
|---------------|-------|
| مستوى المخاطر الكامنة | inherent risk; alias `تصنيف المخاطر الكامنة` |
| تصنيف المخاطر المتبقية | residual risk; alias `مستوى المخاطر المتبقية` |
| تاريخ خطة الالتزام | year values; aliases `السنوات` / `السنة` |
| تاريخ التصحيح السنوي المستهدف | year boxes (same strip style as plan date) |
| تاريخ التصحيح المستهدف - الربعي | quarter boxes (`YYYY-Qn`) |
| تاريخ التصحيح السنوي الفعلي | year boxes |
| تاريخ التصحيح الفعلي - الربعي | quarter boxes (`YYYY-Qn`) |
| تاريخ التصحيح المستهدف | aging fallback |
| تاريخ التصحيح المعدل | aging fallback; also `تاريخ التصحيح المستهدف المعدل` |
...
| الخطة التصحيحية | — |
| ملاحظات الإدارة | — |
| البنود/المتطلبات غير الملتزم بها | aliases `ملاحظات الإلتزام` / `ملاحظات الالتزام` |
